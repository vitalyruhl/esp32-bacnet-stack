...
- Build the serial-only object-list client example:
  `pio run -d examples/esp32/client/basic-object-list-scan -e usb`
- Build the WiFi client demo:
  `pio run -d examples/esp32/client/rich-client/wifi -e usb`
...
- Build the server demo for both configured transports:
  `pio run -d examples/esp32/server/bacnet-server -e usb -e eth`
...
`.github/workflows/pio-tests.yml`, `.github/dependabot.yml`,
`tests/esp32/legacy-basictest/README`, `examples/**/platformio.ini`,
`tests/hil/esp32/wago-client-acceptance/README.md`.
