...
#if defined(HIL_IO_SERVER_COV_MODE) && HIL_IO_SERVER_COV_MODE
#ifndef HIL_IO_SERVER_TARGET_IP
#error "HIL_IO_SERVER_TARGET_IP is required for the I/O-server COV profile"
#endif
#ifndef HIL_IO_SERVER_TARGET_DEVICE_INSTANCE
#error "HIL_IO_SERVER_TARGET_DEVICE_INSTANCE is required for the I/O-server COV profile"
#endif
#undef BACNET_TARGET_IP
#undef BACNET_TARGET_ADDRESS
#undef BACNET_TARGET_DEVICE_INSTANCE
#define BACNET_TARGET_IP HIL_IO_SERVER_TARGET_IP
#define BACNET_TARGET_ADDRESS BACNET_TARGET_IP
#define BACNET_TARGET_DEVICE_INSTANCE HIL_IO_SERVER_TARGET_DEVICE_INSTANCE
#endif
...
// Local-only profile for the ESP32 I/O server. It performs COV-only
// acceptance checks without polling.
#ifndef HIL_IO_SERVER_COV_MODE
#define HIL_IO_SERVER_COV_MODE false
#endif
...
bool connectNetwork() {
  ...
  if (!bacnet_example::EthernetNetwork::begin("bacnet-hil-cov", config) ||
...
ScenarioOutcome runSubscribeCovScenario() {
#if !HIL_IO_SERVER_COV_MODE
  printResult("I", "SubscribeCOV scenario requires a target-specific HIL profile");
  return ScenarioOutcome::Fail;
#else
  if (!ensureRuntimeReady()) {
    return ScenarioOutcome::Fail;
  ...
  const auto waitForSubscriptionResponse = [](bool propertySubscription,
                                              uint8_t invokeId) {
    const uint32_t deadline = millis() + kCovResponseTimeoutMs;
    while (static_cast<int32_t>(millis() - deadline) < 0) {
      const BacnetSubscribeCovResponseKind response = propertySubscription
                                                        ? client.pollSubscribeCovProperty(invokeId)
                                                        : client.pollSubscribeCov(invokeId);
      if (response == BacnetSubscribeCovResponseKind::Ack) {
        Serial.print("[HIL] COV subscription ACK invoke=");
        Serial.println(invokeId);
        return true;
      }
      if (response == BacnetSubscribeCovResponseKind::Error ||
        ...
        Serial.print("[HIL] COV subscription response failure kind=");
        Serial.print(static_cast<uint8_t>(response));
        Serial.print(" invoke=");
        Serial.println(invokeId);
        return false;
      }
    ...
    Serial.print("[HIL] COV subscription response timeout invoke=");
    Serial.println(invokeId);
    return false;
  };
  const auto hasProperty = [](const BacnetCovNotification& notification,
                              BacnetPropertyId property) {
    for (size_t index = 0; index < notification.propertyCount; ++index) {
      if (notification.properties[index].property == property) {
        return true;
      }
    }
    return false;
  ...
  const auto cancelObject = [&](uint32_t processId,
                                BacnetObjectId object,
                                bool confirmed,
                                uint8_t invokeId) {
    return client.sendSubscribeCov(target, processId, object, 0, invokeId, confirmed) &&
           waitForSubscriptionResponse(false, invokeId);
  };
  const auto cancelProperty = [&](uint32_t processId,
                                  BacnetObjectId object,
                                  BacnetPropertyId property,
                                  uint8_t invokeId) {
    return client.sendSubscribeCovProperty(target, processId, object, property, 0, invokeId) &&
           waitForSubscriptionResponse(true, invokeId);
  };

  const BacnetObjectId setButton{static_cast<uint16_t>(BacnetObjectType::BinaryInput), 2};
  const BacnetObjectId midButton{static_cast<uint16_t>(BacnetObjectType::BinaryInput), 1};
  const BacnetObjectId resetButton{static_cast<uint16_t>(BacnetObjectType::BinaryInput), 0};
  const BacnetObjectId lightSensor{static_cast<uint16_t>(BacnetObjectType::AnalogInput), 0};
  BacnetCovNotification notification;

  constexpr uint32_t kSetProcessId = 0xE202U;
  logSubscription("A", kSetProcessId, setButton, "object", false, 21);
  if (!client.sendSubscribeCov(target, kSetProcessId, setButton, 30, 21, false) ||
      !waitForSubscriptionResponse(false, 21) ||
      !waitForNotification(kCovResponseTimeoutMs, notification) || notification.confirmed ||
      notification.object.type != setButton.type || notification.object.instance != setButton.instance ||
      !hasProperty(notification, BacnetPropertyId::PresentValue) ||
      !hasProperty(notification, BacnetPropertyId::StatusFlags)) {
    printResult("FAIL", "COV A object initial notification");
    return ScenarioOutcome::Fail;
  }
  printResult("PASS", "COV A object initial Present_Value + Status_Flags");
  Serial.println("[HIL] Press SET (BI2) within 20 seconds; no ReadProperty is sent.");
  if (!waitForNotification(kCovManualChangeTimeoutMs, notification) || notification.confirmed ||
      !hasProperty(notification, BacnetPropertyId::PresentValue) ||
      !hasProperty(notification, BacnetPropertyId::StatusFlags)) {
    printResult("FAIL", "COV A button notification");
    return ScenarioOutcome::Fail;
  }
  if (!client.sendSubscribeCov(target, kSetProcessId, setButton, 60, 22, false) ||
      !waitForSubscriptionResponse(false, 22) ||
      !waitForNotification(kCovResponseTimeoutMs, notification) ||
      !cancelObject(kSetProcessId, setButton, false, 23) ||
      waitForNotification(kCovCancellationQuietMs, notification)) {
    printResult("FAIL", "COV A renewal or cancellation");
    return ScenarioOutcome::Fail;
  }
  printResult("PASS", "COV A renewal and cancellation");

  constexpr uint32_t kMidProcessId = 0xE201U;
  logSubscription("B", kMidProcessId, midButton, "object", true, 31);
  if (!client.sendSubscribeCov(target, kMidProcessId, midButton, 30, 31, true) ||
      !waitForSubscriptionResponse(false, 31) ||
      !waitForNotification(kCovResponseTimeoutMs, notification) || !notification.confirmed ||
      !hasProperty(notification, BacnetPropertyId::PresentValue) ||
      !hasProperty(notification, BacnetPropertyId::StatusFlags)) {
    printResult("FAIL", "COV B confirmed initial notification");
    return ScenarioOutcome::Fail;
  }
  printResult("PASS", "COV B confirmed initial notification acknowledged");
  Serial.println("[HIL] Press MID (BI1) within 20 seconds; client automatically SimpleACKs COV.");
  if (!waitForNotification(kCovManualChangeTimeoutMs, notification) || !notification.confirmed ||
      !hasProperty(notification, BacnetPropertyId::PresentValue) ||
      !hasProperty(notification, BacnetPropertyId::StatusFlags) ||
      !cancelObject(kMidProcessId, midButton, true, 32)) {
    printResult("FAIL", "COV B confirmed button notification or cancellation");
    return ScenarioOutcome::Fail;
  }
  printResult("PASS", "COV B confirmed button notification acknowledged");

  constexpr uint32_t kResetProcessId = 0xE200U;
  logSubscription("C", kResetProcessId, resetButton, "Present_Value property", false, 41);
  if (!client.sendSubscribeCovProperty(target, kResetProcessId, resetButton, BacnetPropertyId::PresentValue, 30, 41) ||
      !waitForSubscriptionResponse(true, 41) ||
      !waitForNotification(kCovResponseTimeoutMs, notification) || notification.confirmed ||
      notification.propertyCount != 1 ||
      !hasProperty(notification, BacnetPropertyId::PresentValue)) {
    printResult("FAIL", "COV C property initial notification");
    return ScenarioOutcome::Fail;
  }
  Serial.println("[HIL] Press RST (BI0) within 20 seconds; only Present_Value is subscribed.");
  if (!waitForNotification(kCovManualChangeTimeoutMs, notification) ||
      notification.propertyCount != 1 ||
      !hasProperty(notification, BacnetPropertyId::PresentValue) ||
      !cancelProperty(kResetProcessId, resetButton, BacnetPropertyId::PresentValue, 42)) {
    printResult("FAIL", "COV C property button notification or cancellation");
    return ScenarioOutcome::Fail;
  }
  printResult("PASS", "COV C property-only notification");

  constexpr uint32_t kLightProcessId = 0xE100U;
  logSubscription("D", kLightProcessId, lightSensor, "Present_Value property + COV_Increment", false, 51);
  if (!client.sendSubscribeCovProperty(target, kLightProcessId, lightSensor, BacnetPropertyId::PresentValue, 30, 51, false, BacnetClient::kNoArrayIndex, true, 0.5F) ||
      !waitForSubscriptionResponse(true, 51) ||
      !waitForNotification(kCovResponseTimeoutMs, notification) ||
      notification.propertyCount != 1 ||
      !hasProperty(notification, BacnetPropertyId::PresentValue)) {
    printResult("FAIL", "COV D light initial notification");
    return ScenarioOutcome::Fail;
  }
  Serial.println("[HIL] Change LDR light level by at least 0.5 percent within 20 seconds.");
  if (!waitForNotification(kCovManualChangeTimeoutMs, notification) ||
      notification.propertyCount != 1 ||
      !hasProperty(notification, BacnetPropertyId::PresentValue) ||
      !cancelProperty(kLightProcessId, lightSensor, BacnetPropertyId::PresentValue, 52)) {
    printResult("FAIL", "COV D light COV_Increment notification or cancellation");
    return ScenarioOutcome::Fail;
  }
  printResult("PASS", "COV D property COV_Increment notification");
...
#else
#if HIL_IO_SERVER_COV_MODE
  runScenario(summary, "S07", "ESP32 I/O-server SubscribeCOV", true, true, runSubscribeCovScenario, "disabled");

  const ScenarioOutcome finalOutcome = summary.fail > 0 || summary.requiredFailed > 0
                                         ? ScenarioOutcome::Fail
                                         : ScenarioOutcome::Pass;
