...
try {
    ...
    Write-BacnetExampleHeader -Title 'BACnet native example' -BindAddress $resolved.BindAddress -Target $resolved.Target -DeviceId $resolved.DeviceId -ObjectType 'binary-value' -Instance $instance -TimeoutMs $resolved.TimeoutMs
    $result = Invoke-BacnetNativeCapture $exe @('--bind',$resolved.BindAddress,'--target',$resolved.Target,'--device-id',$resolved.DeviceId,'--timeout-ms',$resolved.TimeoutMs,'read',("BV{0}.present-value" -f $instance)) -DryRun:$DryRun
    Assert-BacnetExitCode $result.ExitCode 'BV Present Value read'
    if (-not $DryRun) { Write-Host ("BV{0} Present Value = {1}" -f $instance, ($result.Output -join "`n")) }
    exit 0
