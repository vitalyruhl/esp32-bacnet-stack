...
bool parseArguments(int argc, char* argv[], BacnetNativeCliOptions& options) {
  for (int index = 1; index < argc; ++index) {
    ...
    if (std::strcmp(argument, "--bind") == 0 && !options.hasBind && index + 1 < argc) {
    ...
    } else if (std::strcmp(argument, "--device-id") == 0 && !options.hasDeviceId && index + 1 < argc) {
      ...
      return false;
    if ((!options.hasBind && std::strcmp(argument, "--bind") == 0) || (!options.hasBroadcast && std::strcmp(argument, "--broadcast") == 0) || (!options.hasDeviceId && std::strcmp(argument, "--device-id") == 0))
      return false;
  }
...
bool resolveInterface(BacnetNativeCliOptions& options) {
  ...
  const WindowsInterfaceSelectionStatus status = windowsDiscoverIpv4Interface(
    options.hasBind ? &options.bindEndpoint : nullptr, selected, candidates, sizeof(candidates) / sizeof(candidates[0]), candidateCount);
  if (status != WindowsInterfaceSelectionStatus::Selected) {
    if (status == WindowsInterfaceSelectionStatus::Multiple) {
      for (size_t index = 0; index < candidateCount; ++index) {
        std::fputs("Candidate: ", stderr);
        std::fputs(candidates[index].name, stderr);
        std::fputs(" bind=", stderr);
        printEndpoint(stderr, candidates[index].address);
        std::fputs(" broadcast=", stderr);
        printEndpoint(stderr, windowsBroadcastAddress(candidates[index].address, candidates[index].netmask));
        std::fputc('\n', stderr);
      }
  ...
  std::fputs("Auto-selected interface: ", stderr);
  std::fputs(selected.name, stderr);
  std::fputs("\nBind: ", stderr);
  printEndpoint(stderr, options.bindEndpoint);
  std::fputs("\nBroadcast: ", stderr);
  printEndpoint(stderr, options.broadcastEndpoint);
  std::fputc('\n', stderr);
  return true;
}
...
const char* metadataValue(BacnetClient& client, const BacnetIpEndpoint& endpoint, uint32_t deviceId, BacnetPropertyId property, uint32_t timeoutMs, char* buffer, size_t capacity) {
  ...
  if (status == BacnetNativeReadStatus::Ack) {
    ...
    std::snprintf(buffer, capacity, "<%s>", bacnetNativeReadStatusText(status));
  }
  return buffer;
}
...
void printObjectCounts(BacnetClient& client, const BacnetIpEndpoint& endpoint, uint32_t deviceId, uint32_t timeoutMs) {
  uint32_t counts[20] = {};
  BacnetValue value;
  if (bacnetNativeReadProperty(client, endpoint, BacnetPropertyRequest{BacnetObjectId{8, deviceId}, BacnetPropertyId::ObjectList, 0}, timeoutMs, value) != BacnetNativeReadStatus::Ack || value.type != BacnetValueType::Unsigned) {
    std::puts("objects=<unsupported>");
    return;
  }
  const uint32_t objectCount = value.unsignedValue;
  for (uint32_t index = 1; index <= objectCount; ++index) {
    if (bacnetNativeReadProperty(client, endpoint, BacnetPropertyRequest{BacnetObjectId{8, deviceId}, BacnetPropertyId::ObjectList, index}, timeoutMs, value) == BacnetNativeReadStatus::Ack && value.type == BacnetValueType::ObjectIdentifier && value.objectValue.type < 20)
      ++counts[value.objectValue.type];
  }
...
int main(int argc, char* argv[]) {
  ...
  if (!options.hasBind) {
    ...
    WindowsIpv4Interface candidates[16] = {};
    size_t candidateCount = 0;
    windowsDiscoverIpv4Interface(nullptr, selected, candidates, sizeof(candidates) / sizeof(candidates[0]), candidateCount);
    if (candidateCount == 0) {
      std::fputs("[E] No suitable active IPv4 interface found.\n", stderr);
      return 1;
    }
    ...
    for (size_t candidateIndex = 0; candidateIndex < candidateCount; ++candidateIndex) {
      ...
      if (!runtime.begin() || !transport.setBindAddress(candidate.address))
        continue;
      BacnetClient client(transport, &clock);
      if (!client.begin() || !client.sendWhoIs(broadcast)) {
        client.end();
        continue;
      }
      usedInterface = true;
      ...
      while (client.nowMs() - start < options.timeoutMs) {
        ...
        if (client.pollIAm(device) && (!options.hasDeviceId || device.deviceInstance == options.deviceId)) {
          ...
          if (!duplicate && deviceCount < sizeof(devices) / sizeof(devices[0]))
  ...
  while (client.nowMs() - start < options.timeoutMs) {
    ...
    if (client.pollIAm(device) && (!options.hasDeviceId || device.deviceInstance == options.deviceId)) {
      ...
      printObjectCounts(client, device.endpoint, device.deviceInstance, options.timeoutMs);
      found = true;
      break;
    }
    client.idle();
