...
bool parseObjectType(const char* text, size_t length, BacnetObjectType& type) {
  if (length == 2 && (std::toupper(text[0]) == 'A') && std::toupper(text[1]) == 'I') {
    type = BacnetObjectType::AnalogInput;
    return true;
  }
  if (length == 2 && (std::toupper(text[0]) == 'A') && std::toupper(text[1]) == 'O') {
    type = BacnetObjectType::AnalogOutput;
    return true;
  }
  if (length == 2 && (std::toupper(text[0]) == 'A') && std::toupper(text[1]) == 'V') {
    type = BacnetObjectType::AnalogValue;
    return true;
  }
  if (length == 2 && (std::toupper(text[0]) == 'B') && std::toupper(text[1]) == 'I') {
    type = BacnetObjectType::BinaryInput;
    return true;
  }
  if (length == 2 && (std::toupper(text[0]) == 'B') && std::toupper(text[1]) == 'O') {
    type = BacnetObjectType::BinaryOutput;
    return true;
  }
  if (length == 2 && (std::toupper(text[0]) == 'B') && std::toupper(text[1]) == 'V') {
    type = BacnetObjectType::BinaryValue;
    return true;
  }
  if (length == 3 && std::toupper(text[0]) == 'M' && std::toupper(text[1]) == 'S' && std::toupper(text[2]) == 'I') {
    type = BacnetObjectType::MultiStateInput;
    return true;
  }
  if (length == 3 && std::toupper(text[0]) == 'M' && std::toupper(text[1]) == 'S' && std::toupper(text[2]) == 'O') {
    type = BacnetObjectType::MultiStateOutput;
    return true;
  }
  if (length == 3 && std::toupper(text[0]) == 'M' && std::toupper(text[1]) == 'S' && std::toupper(text[2]) == 'V') {
    type = BacnetObjectType::MultiStateValue;
    return true;
  }
  if (length == 6 && std::toupper(text[0]) == 'D' && std::toupper(text[1]) == 'E' && std::toupper(text[2]) == 'V' && std::toupper(text[3]) == 'I' && std::toupper(text[4]) == 'C' && std::toupper(text[5]) == 'E') {
...
bool bacnetNativeParseEndpoint(const char* text, BacnetIpEndpoint& endpoint) {
  ...
  if (!parseEndpointPort(text, colon))
    return bacnetCliParseIpv4(text, endpoint);
  char address[16] = {};
  ...
  uint32_t port = 0;
  if (!bacnetCliParseIpv4(address, endpoint) || !bacnetCliParseUnsigned(colon + 1, port) || port == 0 || port > UINT16_MAX)
    return false;
...
bool bacnetNativeParseObjectSelector(const char* text, BacnetObjectSelector& selector) {
  ...
  while (std::isalpha(static_cast<unsigned char>(text[prefixLength])))
    ++prefixLength;
  if (prefixLength == 0 || text[prefixLength] == '\0')
    return false;
  ...
  if (!parseObjectType(text, prefixLength, type) ||
      ...
      instance > kMaximumDeviceInstance)
    return false;
...
bool bacnetNativeParseProperty(const char* text, BacnetPropertyId& property) {
  ...
  static constexpr Property properties[] = {
    {"object-identifier", BacnetPropertyId::ObjectIdentifier}, {"objectIdentifier", BacnetPropertyId::ObjectIdentifier}, {"object-name", BacnetPropertyId::ObjectName}, {"objectName", BacnetPropertyId::ObjectName}, {"description", BacnetPropertyId::Description}, {"present-value", BacnetPropertyId::PresentValue}, {"presentValue", BacnetPropertyId::PresentValue}, {"status-flags", BacnetPropertyId::StatusFlags}, {"statusFlags", BacnetPropertyId::StatusFlags}, {"event-state", BacnetPropertyId::EventState}, {"reliability", BacnetPropertyId::Reliability}, {"out-of-service", BacnetPropertyId::OutOfService}, {"outOfService", BacnetPropertyId::OutOfService}, {"priority-array", BacnetPropertyId::PriorityArray}, {"priorityArray", BacnetPropertyId::PriorityArray}, {"units", BacnetPropertyId::Units}, {"vendor-name", BacnetPropertyId::VendorName}, {"vendorName", BacnetPropertyId::VendorName}, {"vendor-id", BacnetPropertyId::VendorIdentifier}, {"vendorIdentifier", BacnetPropertyId::VendorIdentifier}, {"model-name", BacnetPropertyId::ModelName}, {"modelName", BacnetPropertyId::ModelName}, {"firmware-revision", BacnetPropertyId::FirmwareRevision}, {"firmwareRevision", BacnetPropertyId::FirmwareRevision}, {"application-software-version", BacnetPropertyId::ApplicationSoftwareVersion}, {"applicationSoftwareVersion", BacnetPropertyId::ApplicationSoftwareVersion}, {"object-type", BacnetPropertyId::ObjectType}, {"objectType", BacnetPropertyId::ObjectType}, {"protocol-version", BacnetPropertyId::ProtocolVersion}, {"protocolVersion", BacnetPropertyId::ProtocolVersion}, {"protocol-revision", BacnetPropertyId::ProtocolRevision}, {"protocolRevision", BacnetPropertyId::ProtocolRevision}, {"object-list", BacnetPropertyId::ObjectList}, {"objectList", BacnetPropertyId::ObjectList}, {"property-list", BacnetPropertyId::PropertyList}, {"propertyList", BacnetPropertyId::PropertyList}};
  const auto candidate = std::find_if(
    std::begin(properties), std::end(properties), [text](const Property& entry) { return equalsIgnoreCase(text, entry.name); });
  if (candidate == std::end(properties))
    return false;
...
bool bacnetNativeParseObjectPropertySelector(const char* text,
  ...
  if (indexLength == 0 || indexLength >= sizeof(indexText))
    return false;
  ...
  if (!bacnetNativeParseObjectSelector(object, selector) ||
      ...
      !bacnetCliParseUnsigned(indexText, parsedIndex))
    return false;
...
const char* bacnetNativeObjectToken(BacnetObjectId object, char* buffer, size_t capacity) {
  ...
  if (buffer == nullptr || capacity == 0 || std::snprintf(buffer, capacity, "%s%lu", prefix, static_cast<unsigned long>(object.instance)) <= 0)
    return nullptr;
  return buffer;
}
...
const char* bacnetNativeReadStatusText(BacnetNativeReadStatus status) {
  switch (status) {
    case BacnetNativeReadStatus::Ack:
      return "ok";
    case BacnetNativeReadStatus::Error:
      return "error";
    case BacnetNativeReadStatus::Reject:
      return "reject";
    case BacnetNativeReadStatus::Abort:
      return "abort";
    case BacnetNativeReadStatus::Timeout:
      return "timeout";
    case BacnetNativeReadStatus::DecodeError:
      return "decode-error";
    case BacnetNativeReadStatus::SendFailed:
      return "send-failed";
  }
...
BacnetCliExitCode bacnetNativeReadExitCode(BacnetNativeReadStatus status) {
  switch (status) {
    case BacnetNativeReadStatus::Ack:
      return BacnetCliExitCode::Success;
    case BacnetNativeReadStatus::Timeout:
      return BacnetCliExitCode::Timeout;
    case BacnetNativeReadStatus::Reject:
      return BacnetCliExitCode::Reject;
    case BacnetNativeReadStatus::Abort:
      return BacnetCliExitCode::Abort;
    case BacnetNativeReadStatus::DecodeError:
      return BacnetCliExitCode::DecodeError;
    default:
      return BacnetCliExitCode::RuntimeFailure;
  }
}
...
bool bacnetNativeResolveDevice(BacnetClient& client, WindowsBacnetDatagramTransport& transport, const BacnetNativeCliOptions& options, BacnetIpEndpoint& endpoint) {
  ...
  if (!options.hasDeviceId || !options.hasBroadcast)
    return false;
  ...
  while (client.nowMs() - startedAt < options.timeoutMs) {
    ...
    if (client.pollIAm(device) && device.deviceInstance == options.deviceId) {
...
BacnetNativeReadStatus bacnetNativeReadProperty(BacnetClient& client, const BacnetIpEndpoint& endpoint, BacnetPropertyRequest request, uint32_t timeoutMs, BacnetValue& value) {
  ...
  if (!client.sendReadProperty(endpoint, request, kInvokeId))
    return BacnetNativeReadStatus::SendFailed;
  const uint32_t startedAt = client.nowMs();
  while (client.nowMs() - startedAt < timeoutMs) {
    switch (client.pollReadPropertyStatus(value, kInvokeId, request)) {
      case BacnetReadPropertyPollStatus::Ack:
        return BacnetNativeReadStatus::Ack;
      case BacnetReadPropertyPollStatus::Error:
        return BacnetNativeReadStatus::Error;
      case BacnetReadPropertyPollStatus::Reject:
        return BacnetNativeReadStatus::Reject;
      case BacnetReadPropertyPollStatus::Abort:
        return BacnetNativeReadStatus::Abort;
      case BacnetReadPropertyPollStatus::DecodeError:
        return BacnetNativeReadStatus::DecodeError;
