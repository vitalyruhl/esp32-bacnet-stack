...
bool parseArguments(int argc,
  ...
  for (int index = 1; index < argc; ++index) {
    ...
    if (std::strcmp(argument, "list") == 0 && command == Command::None)
      command = Command::List;
    else if (std::strcmp(argument, "read") == 0 && command == Command::None)
      command = Command::Read;
    else if (std::strcmp(argument, "subscribe") == 0 && command == Command::None)
      command = Command::Subscribe;
    else if (std::strcmp(argument, "priority-slot") == 0 && command == Command::None)
      command = Command::PrioritySlot;
    else if (std::strcmp(argument, "write-bv-priority") == 0 && command == Command::None)
      command = Command::WriteBvPriority;
    else if (std::strcmp(argument, "relinquish-bv-priority") == 0 && command == Command::None)
      command = Command::RelinquishBvPriority;
    else if (std::strcmp(argument, "--bind") == 0 && !options.hasBind && index + 1 < argc)
      options.hasBind = bacnetCliParseIpv4(argv[++index], options.bindEndpoint);
    else if (std::strcmp(argument, "--broadcast") == 0 && !options.hasBroadcast && index + 1 < argc)
      options.hasBroadcast = bacnetCliParseIpv4(argv[++index], options.broadcastEndpoint);
    else if (std::strcmp(argument, "--target") == 0 && !options.hasTarget && index + 1 < argc)
      options.hasTarget = bacnetNativeParseEndpoint(argv[++index], options.targetEndpoint);
    else if ((std::strcmp(argument, "--device-id") == 0 || std::strcmp(argument, "-id") == 0) && !options.hasDeviceId && index + 1 < argc)
      options.hasDeviceId = bacnetCliParseUnsigned(argv[++index], options.deviceId) && options.deviceId <= 0x3fffff;
      ...
      if (!bacnetCliParseUnsigned(argv[++index], options.timeoutMs) || options.timeoutMs == 0)
        return false;
    } else if (std::strcmp(argument, "--max") == 0 && index + 1 < argc) {
      if (!bacnetCliParseUnsigned(argv[++index], commandOptions.maximum))
        return false;
    } else if (std::strcmp(argument, "--priority") == 0 && index + 1 < argc) {
      if (!parsePriority(argv[++index], commandOptions))
        return false;
    } else if (std::strcmp(argument, "--lifetime-seconds") == 0 && index + 1 < argc) {
      if (!bacnetCliParseUnsigned(argv[++index], commandOptions.lifetimeSeconds) || commandOptions.lifetimeSeconds == 0)
        return false;
    } else if (std::strcmp(argument, "--duration-seconds") == 0 && index + 1 < argc) {
      if (!bacnetCliParseUnsigned(argv[++index], commandOptions.durationSeconds) || commandOptions.durationSeconds == 0)
        return false;
    } else if (std::strcmp(argument, "--execute") == 0)
      commandOptions.execute = true;
    else if (std::strcmp(argument, "--verbose") == 0)
      options.verbose = true;
    else if (argument[0] != '-' && selectorText == nullptr)
      selectorText = argument;
    else if (argument[0] != '-' && valueText == nullptr && command == Command::WriteBvPriority)
      valueText = argument;
    else
      return false;
  }
  ...
  if (!options.hasBind || !options.hasDeviceId || !hasEndpoint || selectorText == nullptr)
    return false;
  if (command == Command::Read)
    return bacnetNativeParseObjectPropertySelector(
      selectorText,
      commandOptions.selector,
      commandOptions.property,
      commandOptions.arrayIndex);
  if (!bacnetNativeParseObjectSelector(selectorText, commandOptions.selector))
    return false;
  const BacnetObjectType type = static_cast<BacnetObjectType>(commandOptions.selector.object.type);
  if (command == Command::List)
    return true;
  if (command == Command::Subscribe)
    return type == BacnetObjectType::AnalogValue;
  if (command == Command::PrioritySlot || command == Command::RelinquishBvPriority)
    return type == BacnetObjectType::BinaryValue && commandOptions.hasPriority;
  if (command == Command::WriteBvPriority)
    return type == BacnetObjectType::BinaryValue && commandOptions.hasPriority && valueText != nullptr && (std::strcmp(valueText, "active") == 0 || std::strcmp(valueText, "inactive") == 0);
  return false;
}
...
int writeExitCode(BacnetDeviceSessionWriteStatus status) {
  switch (status) {
    ...
    case BacnetDeviceSessionWriteStatus::Reject:
      return static_cast<int>(BacnetCliExitCode::Reject);
    case BacnetDeviceSessionWriteStatus::Abort:
      return static_cast<int>(BacnetCliExitCode::Abort);
    case BacnetDeviceSessionWriteStatus::Disabled:
      return static_cast<int>(BacnetCliExitCode::FeatureDisabled);
...
int runRead(BacnetClient& client, const BacnetIpEndpoint& endpoint, const BacnetNativeCliOptions& options, const CommandOptions& commandOptions) {
  ...
  if (commandOptions.selector.object.type == static_cast<uint16_t>(BacnetObjectType::BinaryValue) &&
    ...
    if (value.unsignedValue == 0) {
      std::puts("inactive");
      return 0;
    }
    if (value.unsignedValue == 1) {
      std::puts("active");
      return 0;
    }
  }
...
int runList(BacnetDeviceSession& session,
  ...
  std::sort(matches.begin(), matches.end(), [](const BacnetScannedObject* left, const BacnetScannedObject* right) {
  ...
  for (size_t index = 0; index < listed; ++index) {
    ...
    if (entry.objectNameStatus != BacnetDeviceSessionReadStatus::Ack)
      ++errors;
    if (entry.descriptionStatus != BacnetDeviceSessionReadStatus::Ack)
      ++errors;
    char token[24] = {};
...
int runPrioritySlot(BacnetDeviceSession& session, const CommandOptions& commandOptions, uint32_t timeoutMs) {
  ...
  if (status != BacnetPropertyReadStatus::Ack && status != BacnetPropertyReadStatus::EmptyValue) {
    std::fprintf(stderr, "[E] priority slot read failed: %s\n", bacnetPropertyReadStatusText(status));
    return static_cast<int>(BacnetCliExitCode::RuntimeFailure);
  }
  char text[BacnetValue::kMaxTextLength] = {};
...
int covExitCode(BacnetCovSubscriptionStatus status) {
  if (status == BacnetCovSubscriptionStatus::Timeout)
    return static_cast<int>(BacnetCliExitCode::Timeout);
  if (status == BacnetCovSubscriptionStatus::Reject)
    return static_cast<int>(BacnetCliExitCode::Reject);
  if (status == BacnetCovSubscriptionStatus::Abort)
    return static_cast<int>(BacnetCliExitCode::Abort);
  return static_cast<int>(BacnetCliExitCode::RuntimeFailure);
}
...
void printCovNotification(const BacnetSubscriptionNotification& notification) {
  if (!notification.hasValue || !notification.valueChanged)
    return;
...
int main(int argc, char* argv[]) {
  ...
  if (!parseArguments(argc, argv, options, command, commandOptions, valueText)) {
    std::fputs("[E] invalid command-line arguments\n", stderr);
    printHelp(stderr);
    return static_cast<int>(BacnetCliExitCode::InvalidArguments);
  }
  if ((command == Command::WriteBvPriority || command == Command::RelinquishBvPriority) && !commandOptions.execute)
    return static_cast<int>(BacnetCliExitCode::WriteNotAuthorized);
  WindowsSocketRuntime runtime;
  WindowsBacnetDatagramTransport transport(runtime);
  WindowsMonotonicClock clock;
  if (!runtime.begin() || !transport.setBindAddress(options.bindEndpoint)) {
  ...
  if (!bacnetNativeResolveDevice(client, transport, options, endpoint)) {
    std::fputs("[E] device resolution timeout or failure\n", stderr);
    client.end();
    return static_cast<int>(BacnetCliExitCode::Timeout);
  }
  int result = 0;
  if (command == Command::Read)
    result = runRead(client, endpoint, options, commandOptions);
  else {
    BacnetDeviceSession session(client, options.deviceId, endpoint);
    if (command == Command::List)
      result = runList(session, commandOptions, options.timeoutMs);
    else if (command == Command::Subscribe)
      result = runSubscribe(session, client, commandOptions, options.timeoutMs);
    else if (command == Command::PrioritySlot)
      result = runPrioritySlot(session, commandOptions, options.timeoutMs);
    else
      result = runWrite(session, commandOptions, valueText, options.timeoutMs, command == Command::RelinquishBvPriority);
  }
  client.end();
  return result;
}
