[CmdletBinding()]
param([string]$BindAddress, [string]$Target, [string]$DeviceId, [string]$Instance, [string]$LifetimeSeconds, [string]$TimeoutMs, [string]$ExeDir, [string]$SettingsPath, [switch]$DryRun)
...
try {
    ...
    $exitCode = Invoke-BacnetNative $exe @('--bind',$resolved.BindAddress,'--target',$resolved.Target,'--device-id',$resolved.DeviceId,'--timeout-ms',$resolved.TimeoutMs,'subscribe',("AV{0}" -f $instance),'--lifetime-seconds',$lifetime) -DryRun:$DryRun
