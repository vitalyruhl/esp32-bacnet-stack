...
bool expectRequest(const uint8_t* expected,
  ...
  const size_t actualSize = BacnetProtocol::buildSubscribeCovRequest(
    actual, sizeof(actual), processId, object, lifetimeSeconds, issueConfirmedNotifications);
...
int main() {
  ...
  bool passed = true;
  passed &= expect(expectRequest(unconfirmed120, sizeof(unconfirmed120), 7, analogValue200, 120, false),
                   "unconfirmed SubscribeCOV request");
  passed &= expect(expectRequest(confirmed120, sizeof(confirmed120), 7, analogValue200, 120, true),
                   "confirmed SubscribeCOV request");
  passed &= expect(expectRequest(lifetimeZero, sizeof(lifetimeZero), 7, analogValue200, 0, false),
                   "zero-lifetime SubscribeCOV request");
  passed &= expect(expectRequest(
                     differentProcessAndObject, sizeof(differentProcessAndObject), 0x1234, BacnetObjectId{static_cast<uint16_t>(BacnetObjectType::BinaryValue), 42}, 120, true),
                   "different process and object identifiers");
  ...
  const size_t propertyRequestSize = BacnetProtocol::buildSubscribeCovPropertyRequest(
    propertyRequest, sizeof(propertyRequest), 12, analogValue200, BacnetPropertyId::PresentValue, 60, false, kBacnetNoArrayIndex, true, 0.5F);
  ...
  passed &= expect(BacnetProtocol::parseCovNotification(
                     ...
                     sizeof(covNotificationWithPriority),
                     parsedNotification) &&
                     parsedNotification.property == BacnetPropertyId::PresentValue &&
                     parsedNotification.value.realValue == 42.5F,
