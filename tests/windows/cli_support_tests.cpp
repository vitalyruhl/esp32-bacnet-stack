...
bool testValueDisplayText() {
  ...
  if (std::strcmp(bacnetValueDisplayText(value, text, sizeof(text)), "null") != 0)
    return false;
  value.type = BacnetValueType::Boolean;
  value.booleanValue = true;
  if (std::strcmp(bacnetValueDisplayText(value, text, sizeof(text)), "true") != 0)
    return false;
  value.type = BacnetValueType::Unsigned;
  value.unsignedValue = 12;
  if (std::strcmp(bacnetValueDisplayText(value, text, sizeof(text)), "12") != 0)
    return false;
  value.type = BacnetValueType::Signed;
  value.signedValue = -3;
  if (std::strcmp(bacnetValueDisplayText(value, text, sizeof(text)), "-3") != 0)
    return false;
  value.type = BacnetValueType::Real;
  value.realValue = 12.5F;
  if (std::strcmp(bacnetValueDisplayText(value, text, sizeof(text)), "12.500") != 0)
    return false;
  value.type = BacnetValueType::Enumerated;
  value.unsignedValue = 7;
  if (std::strcmp(bacnetValueDisplayText(value, text, sizeof(text)), "7") != 0)
    return false;
  value.type = BacnetValueType::CharacterString;
  std::memcpy(value.text, "hello", 5);
  value.textLength = 5;
  if (std::strcmp(bacnetValueDisplayText(value, text, sizeof(text)), "hello") != 0)
    return false;
